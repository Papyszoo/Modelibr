...
Given('texture set {string} exists', async ({ page }, name: string) => {
    ...
    console.log(`[API] Created texture set "${uniqueName}" with ID ${textureSet.id}`);
});
...
When("I drag {string} onto {string}", async ({ page }, sourceName: string, targetName: string) => {
    ...
    const sourceCard = page.locator(`.texture-set-card:has-text("${sourceDisplayName}")`).first();
    const targetCard = page.locator(`.texture-set-card:has-text("${targetDisplayName}")`).first();

    await expect(sourceCard).toBeVisible({ timeout: 10000 });
    await expect(targetCard).toBeVisible({ timeout: 10000 });

    await sourceCard.dragTo(targetCard);
    console.log(`[UI] Dragged "${sourceDisplayName}" onto "${targetDisplayName}"`);
});
...
Then("I should see a merge dialog showing source files", async ({ page }) => {
    ...
    const hasMergeContent = await dialog.locator('.file-channel-mapping, .channel-row, .file-name').first().isVisible().catch(() => false);
    expect(hasMergeContent || await dialog.textContent().then(t => t?.includes('Merge'))).toBeTruthy();
    console.log('[UI] Merge dialog with source files is visible');
});

Then("I should see a merge dialog showing {string}", async ({ page }, fileName: string) => {
    const dialog = page.locator('.p-dialog').first();
...
Then("I should see an RGB dropdown with options {string}", async ({ page }, optionsStr: string) => {
    const rgbDropdown = page.locator('.p-dropdown').first();
    await rgbDropdown.click();
    
    const options = await page.locator('.p-dropdown-item').allTextContents();
    const expectedOptions = optionsStr.split(',').map(s => s.trim());
    
    for (const expected of expectedOptions) {
        const found = options.some(opt => opt.includes(expected));
        expect(found, `Expected option "${expected}" not found in [${options.join(', ')}]`).toBe(true);
    }
...
When("I select {string} for the RGB dropdown", async ({ page }, option: string) => {
    const rgbDropdown = page.locator('.file-channel-mapping .p-dropdown, .channel-row .p-dropdown').first();
    await rgbDropdown.click();
...
Then("I should see R, G, B channel dropdowns appear", async ({ page }) => {
    ...
    for (const channel of ['R:', 'G:', 'B:']) {
        await expect(page.locator(`.channel-row:has-text("${channel}"), label:has-text("${channel}")`).first()).toBeVisible();
    }
    console.log('[UI] R, G, B channel dropdowns are visible');
});

When("I set the {word} channel to {string}", async ({ page }, channel: string, type: string) => {
    ...
    const dropdown = page.locator('.split-channels .channel-row.indent').nth(index).locator('.p-dropdown');
...
When("I set the A channel to {string}", async ({ page }, type: string) => {
    const channelRow = page.locator('.channel-row:has-text("A:"), label:has-text("Alpha")').locator('..');
...
When('I select {string} and set G to {string}', async ({ page }, option: string, type: string) => {
...
Then("I should see a warning about replacing existing texture", async ({ page }) => {
    const warning = page.locator('.override-warning, .warning-message, .p-message-warn');
...
Then("{string} should have AO, Roughness, and Metallic textures", async ({ page }, setName: string) => {
    // Get stored unique name from shared state
    const set = sharedState.getTextureSet(setName);
    const displayName = set?.name || setName;
    
    const card = page.locator(`.texture-set-card:has-text("${displayName}")`).first();
    await card.dblclick();
    await page.waitForSelector('.texture-set-viewer, .p-dialog', { timeout: 10000 });
    
    // Try both "Texture Types" tab (new name) and "Textures" tab (old name)
    const newTab = page.locator('.p-tabview-nav-link:has-text("Texture Types")');
    const oldTab = page.locator('.p-tabview-nav-link:has-text("Textures")');
    
    if (await newTab.isVisible({ timeout: 1000 }).catch(() => false)) {
        await newTab.click();
    } else if (await oldTab.isVisible({ timeout: 1000 }).catch(() => false)) {
        await oldTab.click();
    }
    
    await expect(page.locator('[data-texture-type="AO"], .texture-card:has-text("AO")')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('[data-texture-type="Roughness"], .texture-card:has-text("Roughness")')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('[data-texture-type="Metallic"], .texture-card:has-text("Metallic")')).toBeVisible({ timeout: 5000 });
    console.log(`[UI] "${displayName}" has AO, Roughness, and Metallic textures`);
});

Then("{string} should have an {string} texture", async ({ page }, setName: string, type: string) => {
    // Get stored unique name from shared state
    const set = sharedState.getTextureSet(setName);
    const displayName = set?.name || setName;
    ...
    const typeCard = page.locator(`[data-texture-type="${type}"], .texture-card:has-text("${type}")`);
    await expect(typeCard).toBeVisible();
    console.log(`[UI] "${setName}" has ${type} texture`);
});

Then("{string} should have both {string} and {string} textures", async ({ page }, setName: string, type1: string, type2: string) => {
    ...
    await expect(page.locator(`[data-texture-type="${type1}"], .texture-card:has-text("${type1}")`)).toBeVisible();
    await expect(page.locator(`[data-texture-type="${type2}"], .texture-card:has-text("${type2}")`)).toBeVisible();
    console.log(`[UI] "${setName}" has both ${type1} and ${type2} textures`);
});
...
When('I change Height card mode to {string}', async ({ page }, mode: string) => {
    // Find Height card and click mode dropdown
    const heightCard = page.locator('[data-texture-type="Height"], .texture-card:has-text("Height")');
...
Then('{string} should be the assigned type', async ({ page }, type: string) => {
    const typeIndicator = page.locator(`[data-texture-type="${type}"], .texture-card:has-text("${type}")`);
