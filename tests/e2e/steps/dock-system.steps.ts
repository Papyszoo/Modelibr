...
When(
    ...
    async ({ page }, modelName: string) => {
        const modelData = sharedState.getModel(modelName) || sharedState.getModel(modelName + ".glb") || sharedState.getModel(modelName + ".fbx");
        if (!modelData) {
            throw new Error(`Model "${modelName}" not found in shared state`);
        }
        
        // Find and click on the model in the grid
        const modelCard = page.locator(`.model-card, .model-grid-item, [data-model-name="${modelName}"]`).first();
        ...
        await page.waitForSelector(".model-viewer, .viewer-canvas, .viewer-controls", { 
            state: "visible", 
            timeout: 10000 
        });
...
When("I open Settings in the right panel", async ({ page }) => {
    ...
    const addTabButton = page.locator(".p-splitter-panel").nth(1).locator("button").filter({ hasText: "+" }).first();
    
    // If not visible, look for the add tab button in the dock bar
    const dockAddButton = page.locator('.dock-bar button[aria-label="Add tab"], .add-tab-button').last();
    ...
    await page.waitForSelector(".p-menu, .p-tieredmenu, .dock-menu", { state: "visible", timeout: 5000 });
    await page.locator("text=Settings").click();
    
    // Wait for settings panel
    await page.waitForSelector('text="Application Settings", text="Settings"', { state: "visible", timeout: 5000 });
...
Then("the Settings tab should still be visible", async ({ page }) => {
    ...
    const settingsTab = page.locator('.draggable-tab:has-text("Settings"), [data-tab-type="settings"]');
    
    const isContentVisible = await settingsContent.isVisible({ timeout: 3000 }).catch(() => false);
    const isTabVisible = await settingsTab.isVisible({ timeout: 3000 }).catch(() => false);
...
Then("the Texture Sets tab should be active in the left panel", async ({ page }) => {
    // Check if Texture Sets content is visible
    const textureSetsContent = page.locator('text="Texture Sets", text="No texture sets found"').first();
    await expect(textureSetsContent).toBeVisible({ timeout: 5000 });
    console.log("[UI] Texture Sets tab is active ✓");
});

Then("the Models tab should still exist in the left panel", async ({ page }) => {
...
When("I open the Texture Sets tab in the left panel", async ({ page }) => {
    ...
    const textureSetsTab = page.locator('.draggable-tab:has-text("Texture Sets")');
    
    if (await textureSetsTab.isVisible({ timeout: 2000 }).catch(() => false)) {
        await textureSetsTab.click();
    } else {
        // Add Texture Sets tab via menu
        const addTabButton = page.locator('.dock-bar button[aria-label="Add tab"]').first();
        await addTabButton.click();
...
Then(
    ...
    async ({ page }, expectedCount: number, entityName: string) => {
        ...
        console.log(`[URL] Found ${matchingTabs.length} tab(s) for "${entityName}" (expected ${expectedCount}) ✓`);
    }
);
...
When("I close the Settings tab", async ({ page }) => {
    ...
    const closeButton = settingsTab.locator('button[aria-label="Close"], .close-button, .pi-times').first();
...
When("I open Settings in the right panel again", async ({ page }) => {
    const dockAddButton = page.locator('.dock-bar button[aria-label="Add tab"]').last();
    await dockAddButton.click();
    await page.waitForSelector(".p-menu", { state: "visible", timeout: 5000 });
    await page.locator("text=Settings").click();
    await page.waitForTimeout(500);
...
Then("there should be exactly {int} Settings tab", async ({ page }, expectedCount: number) => {
    const url = page.url();
    const settingsCount = (url.match(/settings/g) || []).length;
    expect(settingsCount).toBe(expectedCount);
    console.log(`[URL] Found ${settingsCount} Settings tab(s) (expected ${expectedCount}) ✓`);
});
...
Then("there should be exactly {int} Texture Sets tab in the left panel", async ({ page }, expectedCount: number) => {
    const url = page.url();
    const leftTabsMatch = url.match(/leftTabs=([^&]*)/);
    const leftTabs = leftTabsMatch ? leftTabsMatch[1].split(",") : [];
    ...
    console.log(`[URL] Found ${textureSetsTabs.length} Texture Sets tab(s) ✓`);
});
...
Then("the URL should not contain duplicate tab IDs", async ({ page }) => {
    ...
    console.log(`[URL] Left unique count: ${leftUnique.size}, actual count: ${leftTabs.length}`);
...
Given("I have {string} open in the right panel", async ({ page }, modelName: string) => {
    const modelData = sharedState.getModel(modelName);
    if (!modelData) {
        throw new Error(`Model "${modelName}" not found in shared state`);
    }
    ...
    const baseUrl = process.env.FRONTEND_URL || "http://localhost:3002";
    await page.goto(`${baseUrl}/?leftTabs=modelList&activeLeft=modelList&rightTabs=model-${modelData.id}&activeRight=model-${modelData.id}`);
    await page.waitForSelector(".p-splitter", { state: "visible", timeout: 15000 });
    console.log(`[UI] Opened "${modelName}" in right panel ✓`);
});

When("I try to open {string} from the model list", async ({ page }, modelName: string) => {
    const modelData = sharedState.getModel(modelName);
    if (!modelData) {
        throw new Error(`Model "${modelName}" not found in shared state`);
    }
    ...
    // Count tabs before
    const url = page.url();
    tabCountBefore = (url.match(/model-/g) || []).length;
...
Then("there should be exactly {int} tab for {string} in total", async ({ page }, expectedCount: number, modelName: string) => {
    const modelData = sharedState.getModel(modelName);
    if (!modelData) {
        throw new Error(`Model "${modelName}" not found in shared state`);
    }
...
Then("I should see {int} model tabs", async ({ page }, expectedCount: number) => {
    const url = page.url();
    const modelMatches = url.match(/model-\d+/g) || [];
    
    // Filter unique model tabs
    const uniqueModels = new Set(modelMatches.filter(m => m.startsWith("model-")));
...
Given("I have {string} open in another tab", async ({ page }, modelName: string) => {
    console.log(`[Setup] Model "${modelName}" tab assumed open ✓`);
});
...
// Track last closed model for verification
let lastClosedModelId = "";
...
When("I close the {string} tab", async ({ page }, modelName: string) => {
    ...
    lastClosedModelId = modelData ? `model-${modelData.id}` : "";
    
    const tab = page.locator(`.draggable-tab:has-text("${tabLabel}")`);
    const closeButton = tab.locator('.close-button, .pi-times, button[aria-label="Close"]');
...
Then("the {string} tab should become active", async ({ page }, modelName: string) => {
    const modelData = sharedState.getModel(modelName);
    if (!modelData) {
        throw new Error(`Model "${modelName}" not found in shared state`);
    }
    ...
    const url = page.url();
    expect(url).toMatch(new RegExp(`active(Left|Right)=model-${modelData.id}`));
    console.log(`[URL] "${modelName}" tab is now active ✓`);
});

When("I open tabs in order: Texture Sets, Settings, History", async ({ page }) => {
...
Then("the URL parameter {string} should have them in the same order", async ({ page }, paramName: string) => {
    const url = page.url();
    ...
    if (match) {
        ...
        console.log(`[URL] Tabs in correct order: ${tabsWithoutModelList.join(", ")} ✓`);
    }
});
...
Then("the URL should not contain the closed model tab ID", async ({ page }) => {
    ...
    const closedTabExists = allTabs.includes(lastClosedModelId);
    
    expect(closedTabExists).toBe(false);
    console.log(`[URL] Closed tab "${lastClosedModelId}" not in URL ✓`);
});
...
Then(
    ...
    async ({ page }, expectedCount: number, tabId: string) => {
        ...
        console.log(`[RESULT] Found ${matchingTabs.length} occurrence(s) of "${tabId}" (expected ${expectedCount})`);
...
Then("the model viewer should be visible in the right panel", async ({ page }) => {
