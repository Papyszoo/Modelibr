...
export class EnvironmentMapsPage {
  ...
  async getToolbarCountLabel(): Promise<string> {
    ...
    // shared primitives.
    const label =
      (await this.page.locator(".list-toolbar-count span").textContent()) ?? "";
    return label.trim();
  }
  ...
  getEnvironmentMapCardByName(name: string) {
    return this.page.locator(".environment-map-card").filter({
  ...
  async dragAndDropUpload(
    ...
    await this.listRoot.evaluate((element, files) => {
  ...
  async waitForEnvironmentMapByName(
    ...
    const findCard = async (): Promise<boolean> => {
      ...
      const scrollContainer = this.page.locator(".environment-map-list-main");
      ...
      for (let pos = 0; pos <= scrollHeight; pos += step) {
        await scrollContainer.evaluate((el, y) => el.scrollTo(0, y), pos);
   ...
   *
   * Every scenario here uploads before it clicks, and the window does not go away on its
   * own - the auto-hide in UploadProgressWindow is deliberately commented out ("keep
   * window open as per requirements"). It floats over the CENTRE of the page, which at
   * the 1280x720 viewport with the categories sidebar open is exactly where the first
   * card's click point is. Playwright then resolves the card, finds it visible, and
   * retries the actionability loop until the timeout because the hit test lands on the
   * window - which it reports as "waiting for element to be visible, enabled and stable",
   * the same words a moving element produces.
   *
   * That wording is why this was read as a VirtuosoGrid recycling problem and answered
   * with a scroll-and-settle for nine nightlies. The failure screenshot from run
   * 32804725455 settles it: five maps, no scrolling, the card present and still, and the
   * File Uploads window sitting on top of it.
   */
  ...
  async getDetailValue(label: string): Promise<string> {
    ...
    return (
      (await row.first().locator("dd").first().textContent()) ?? ""
    ).trim();
  }
  ...
  async waitForCardThumbnailLoaded(
    name: string,
    timeout = 30000,
  ): Promise<void> {
  ...
  async getTrackedCardThumbnailTransitions(): Promise<
    CardThumbnailTransitionRecord[]
  > {
  ...
  async getCardThumbnailSrc(name: string): Promise<string> {
    ...
    return (
      (await this.getEnvironmentMapCardThumbnailByName(name).getAttribute(
        "src",
      )) ?? ""
    );
  }
  ...
  private async fillCreateDialog(
    values: EnvironmentMapDialogValues,
  ): Promise<void> {
    if (values.name) {
      await this.uploadDialog
        .locator("#environment-map-name")
        .fill(values.name);
    }
    ...
    if (values.cubeFaces) {
      ...
      const faceIndex: Record<"px" | "nx" | "py" | "ny" | "pz" | "nz", number> =
        {
          px: 0,
          nx: 1,
          py: 2,
    ...
    if (values.thumbnailFile) {
      ...
      await field
        ...
        .setInputFiles(values.thumbnailFile);
      await expect(
        field.locator(".environment-map-upload-file-name"),
  ...
  private async openViewerMenu(menuName: string): Promise<Locator> {
    ...
    const menuLink = menuItem
      .locator(`.p-menuitem-link:has(.p-menuitem-text:text-is("${menuName}"))`)
      .first();
    ...
    await expect(menuItem.locator(".p-submenu-list").first()).toBeVisible({
      timeout: 5000,
    });
    return menuItem;
  }

  private async openViewerPanel(
    menuName: string,
    itemLabel: string,
  ): Promise<void> {
    const panelSelector =
      ...
      for (const candidate of menuCandidates) {
        try {
          ...
          const submenuItem = menuItem
            ...
            .first();
          const submenuVisible = await submenuItem
            .isVisible()
            .catch(() => false);
