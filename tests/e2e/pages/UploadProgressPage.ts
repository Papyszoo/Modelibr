...
export class UploadProgressPage {
   ...
   *
   * The button is addressed through the FloatingWindow header rather than by icon. Every
   * COMPLETED upload row carries its own `pi-times` "Remove" button, so a `.pi-times`
   * inside `#upload-progress-window` is only the close control by accident of DOM order -
   * one batch rendered above the header, or a header restyle, and the old selector would
   * have started removing an upload row and reporting the window closed. `aria-label` was
   * in that selector too and never matched anything: the control carries `title="Close"`.
   */
