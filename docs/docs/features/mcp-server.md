...
These twenty-nine read tools are always available, each a thin wrapper over an
ordinary Modelibr API endpoint - there is no separate search or extraction path:

| Tool | What it does |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| `search_assets` | Full-text + fuzzy-identifier search with structural filters - triangle/vertex/part counts, size (bounding-box dimension), rig (`hasRig`/bone count), materials, UVs, animations, shape class, engine, asset type, and **category**. Every query word is scored on its own, so a document matching more of them ranks higher, and plurals find their singular. Everyday synonyms are matched too, so `rug` reaches a `carpet` and `bookshelf` reaches a `bookcase` - the words a brief is written in rather than the ones a pack happened to use. Conceptual queries (`weapon`, `vehicle`, `building`) hit via deterministic concept labels, ranked below assets whose author actually named them that. Tags and descriptions a person assigned are searched too, ranked with authored names rather than with inferred concepts. Leave the query blank to browse by filters alone. Returns one ranked hit **per asset** (current version only), each carrying a short browse summary **and its structural facts** - triangles, size, parts, materials, UVs, rig, animations - so an agent can compare candidates without a follow-up call per hit. A hit always identifies the **whole, placeable asset**; when the query actually matched a mesh inside it, that mesh is reported separately as `matchedPart`, since `place_asset` places the asset and cannot place a part. The same prop imported twice is **collapsed into one hit**, with the other ids listed as `alsoAt` - a duplicate is still a real asset with its own tags and packs, so it is named rather than hidden. Each hit also carries `media`: the pinned version's thumbnail URL and its readiness, resolved for the whole page in one read and by the same service the scene choice cards use, so search and choices cannot disagree about whether an asset has a preview. The response carries a `query` block explaining itself - see "When a search comes back wrong" below. |
| `search_many` | Several searches in **one call**. A scene brief is a batch of questions - a sofa, a coffee table, a rug, two lamps - and asking them one at a time is a round trip each. Each entry carries its own query and a few filters; the project and profile mode are set once for the whole call. Answers come back in request order with the caller's own label, and an entry that fails carries its own error rather than failing the batch. |
| `get_assets` | The same for metadata: several assets in one call, each at the `versionId` it was asked about. Comparing ten search hits used to be ten round trips. |
| `get_asset` | The derived metadata, part list (each part with its measured `bounds`) and `materialSlots` for an asset, plus `surfaces` - the horizontal faces something can be rested on, largest first, each with an `index` and its height above the asset's base. Read them before stacking: `place_asset(on:)` alone rests a node on the target's _whole-asset_ box top, which is right for a table and wrong for a sofa; pass that `index` as `place_asset(onSurface:)` to sit on the seat instead. Also `suggestedCategories` (deterministic concept-label suggestions the user/agent can confirm-assign). Defaults to the asset's **active** version; pass the `versionId` from a search hit to inspect exactly the version that hit named. |
| `get_part` | A single part's detail, addressed by its part-path (e.g. `/Building/Roof`). Takes the same optional `versionId`. |
| `compute_on_demand` | A cached expensive metric (**local-space** surface area, manifold check, …) keyed by geometry hash, or `pending` if it has not been computed yet. Queue the computation with `analyze_meshes`. The cached `surface-area` carries `"space": "local"` and excludes the scale an instance is placed at - one row is shared by every instance of that geometry; for a particular object's world-space area, read `surfaceArea` off an `analyze_meshes` job. UV overlap and texel density are **not** answerable here either - they depend on the UV layout, which the geometry hash also ignores. |
| `get_index_status` | How much of the library is findable: per family, how many assets are derived, how many have a current search document, and how many were derived under an older projection. An asset can be derived and unindexed - it exists and search cannot see it - so this is what to poll after a bulk import rather than searching for something and guessing. The `notes` name the remedy. |
| `get_facet_ranges` | What the filters mean **in this library**: the real quartiles behind triangles, vertices, parts, materials and size, and the values category, uvStatus, licence, shape, style and theme actually hold, with counts. Worth calling before turning a brief into a filter - "low poly" is a phrase, and whether it means under 2,000 triangles or under 200 depends on the corpus. Quartiles rather than averages, because a handful of sample scenes drags a mean somewhere no asset is. A facet with no values is reported as a labelling gap, not a broken filter. |
...
| `get_import_suggestions` | What the import automation categorized and tagged on its own and nobody has confirmed yet. Each entry says what it decided and the folder it decided it from. Settle them with `review_import_suggestions`. |
| `get_duplicate_assets` | Groups of models that ARE each other: the same meshes under two ids, found by geometry fingerprint rather than by name. A real library import produces these by the hundred - an FBX and an OBJ of one prop land as two assets, and their part names differ so nothing keyed on names could match them. `search_assets` already collapses duplicates within a page of results, so this is for library hygiene: deciding which copy is the real one. |
...
A query that returns junk used to give no signal about **why**, so the next call was a guess
and the one after that was another. Every `search_assets` response now carries a `query`
block that says what the search understood:

- `terms` - the words it actually scored, after stopword removal and abbreviation expansion.
  `SM_Bld_` becomes `building`, `chairs` also matches `chair`.
- `ignored` - the words it dropped, each with a reason: `stopword`, `duplicate`, or
  `beyond-word-limit` (only the first six words are scored, so a brief's intent belongs at
  the front).
- `matches`, per term - **how many assets carry that word on its own**, ignoring every other
  word and every filter. Measured only when the result was thin (three assets or fewer),
  because it costs a query per word and a search that already answered does not need it.
- `didYouMean` - for a word this library has never heard, the nearest names it does hold.
- `note` - one sentence, present only when something needs saying. Read this before
  retrying: "No asset in this library carries 'settee'; the search ran on the rest. The
  nearest names here are: Sofa 01, Loveseat." is the difference between fixing the query and
  inventing another one.

A blank query means "everything matching the filters", and there is nothing to explain, so
the block is absent.
...
| Tool                  | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `search_store_assets` | Search the store catalog for assets that are **not** in your library, by text, item type, tag and format. Every hit carries `alreadyImported`, so a store asset you already pulled in is never offered back to you as a discovery. Against a store that supports it, the search also matches the names of items **inside** a pack and reports which ones matched, with their store item ids - those ids are what `import_store_asset` selects on, so you can take one model out of a thousand-item pack instead of the pack. |
...
| `get_store_import`    | Check a store import started by `import_store_asset` - its status, how many items were created, and what was skipped.                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `get_scenes_using_asset` | Which scenes reference an asset, and how many nodes in each. `get_scene` read backwards. Ask before recycling or replacing one: a scene that points at a deleted asset still loads and still names it, showing a node that will never render. It is also what makes an asset part of a project without being a member of it - a project's asset list unions in whatever its scenes use.                 |
...
Set `MCP_WRITE_ENABLED=true` in your root `.env` and forty-six more tools appear,
...
| `analyze_meshes`            | Measure a model with Blender - UV overlap, texel density, surface area (world-space on the job, local-space in the shared cache), watertightness. Changes nothing. Returns a job id; collect it with `get_job_status`.                                                                                                                                                                     |
| `convert_model`             | Convert a model to another format with Blender - `glb`, `fbx` or `stl` - written as a **new, inactive version**. `gltf` and `obj` are refused: each keeps part of itself in a sidecar file and a version holds one. Returns a job id; collect it with `get_job_status`.                                                                                                                    |
...
| `review_import_suggestions` | Settle what the import automation guessed, in bulk: `accept=true` keeps the categories and tags it applied, `accept=false` takes them back. Omit `modelIds` to settle everything waiting; that form is bounded per call, so repeat while `remaining` is above zero. Taking back never undoes a decision a person made since.                                                               |
| `collapse_duplicate_assets` | Keep one copy of a same-geometry group and **recycle** the rest. You name the survivor - the two copies of a prop are usually an FBX and an OBJ, and only the user knows which their pipeline reads. Every id is re-checked against the survivor's fingerprint first, so a stale listing cannot delete a different asset. Recycled, not merged: `restore_asset` brings one back untouched. |
...
`import_store_asset` is the only tool that reaches off the machine to _acquire_ something,
...
mind when planning a scene: _an agent can fetch a free asset by itself, but never a paid
one._
...
- **The engines are reconciled, not resolved.** A project is commonly Blender
  _and_ Unity, and they disagree about up axis and handedness. The brief lists
...
| `request_upload_ticket`  | For an agent that is **not** on the server: a single-use ticket plus the exact endpoint, field names and a worked example for uploading any asset family over HTTP. For `Model` it also returns `alternatives` - the multi-file and zip routes a loose `.gltf` needs, since `POST /models` alone imports it without its geometry. Pass `textureSetId` to add one more channel to a material you already created. |
...
| `delete_scene`              | Delete a scene. Dry run by default; pass `dryRun=false` to apply.                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `place_asset`               | Place one asset. `groundSnap` rests its base on the floor using the asset's measured origin - use it rather than guessing a Y, or a centered-origin asset lands buried to its middle. `on` rests it on another node instead, `onSurface` names which of that node's resting surfaces, and `faceToward` aims it at a point.                                                                                                                                                                                           |
| `place_primitive`           | Put a blockout box, plane, sphere, cylinder or cone into a scene, with an optional colour - shape before assets, and no library search needed.                                                                                                                                                                                                                                                                                                                                                                       |
| `create_room`               | Emit a whole room shell - floor, four walls and optionally a ceiling - from three numbers, in one write.                                                                                                                                                                                                                                                                                                                                                                                                             |
| `place_assets_batch`        | Place a whole layout of different assets in one write - a sofa, a table, two lamps, a rug. Entries speak exactly `place_asset`'s vocabulary and are applied in array order, so one may rest `on` a node an earlier entry created. Nothing is written unless every entry is valid, and undo takes the layout back out whole.                                                                                                                                                                                          |
...
| `set_lighting_preset`       | Light a scene correctly in one call. Use it before `set_light`: lighting is the thing you cannot check by reading the document back.                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `set_scene_recommendations` | State which candidate you would go with in each slot, without choosing any of them. Replaces the whole set in one write; a slot left out becomes unrecommended. Carries an optional one-to-three-sentence summary the user reads verbatim.                                                                                                                                                                                                                                                                           |
...
whole advice in one write - `[{slotId, candidateId}, …]` plus an optional `summary` about
the direction the set takes and what it trades away.

It **resolves nothing**: no node moves, no slot settles, and `resolvedBy` is untouched. The
separation is the point. Before it, the only way to hint was to put a favourite first, and
card order is not a recommendation - the UI does not read it as one, and inferring intent
from array position would turn an implementation detail into stated advice.

The set is replaced, not merged, so an agent proposing slot by slot can state one coherent
combination at the end rather than leaving a trail of per-slot hints. Every candidate must
still be open; recommending something the user already rejected would put their own
rejection back in front of them as advice. A recommendation _survives_ its candidate being
rejected afterwards - kept as history so the panel can still say what was advised - but it
stops being something a bulk accept can act on.

In the editor, a **Scene intent & recommendations** section sits above the cards with the
scene's description, the summary, the open/settled/rejected counts and each recommended
`slot/candidate` with the rationale it was proposed with. Nothing there is composed in the
browser, and no private deliberation is requested or stored. When two or more unresolved
slots have an open recommendation, **Accept N recommendations** confirms the exact mappings
in a dialog and settles them in one write - recorded as `resolvedBy: "user"`, because the
person clicked. There is deliberately no agent-facing version of that verb.

A candidate can also come from the **Asset Store**, when the library genuinely has nothing
that fits. Such a card is marked _not in your library_, carries the store's own picture and
...
- **Surface area** - exact. Measured twice, because the two answers are for two different
  questions: `surfaceArea` on the job is **world-space**, with the object's scale applied,
  and the cached value is **local-space**, measured in the mesh's own coordinates.
...
##### Converting a model to another format

`convert_model` re-writes a model in a different format, as a **new, inactive version** -
the uploaded file is untouched, exactly as with an unwrap.

```
convert_model(modelId: 812, format: "glb", idempotencyKey: "convert-812-1")
  -> { status: "queued", jobId: 118 }

get_job_status(jobId: 118, waitSeconds: 120)
  -> { status: "Done", result: { versionId: 44, format: "glb", sourceFormat: "fbx",
         fileName: "chair.glb", setAsActive: false } }
```

The targets are `glb`, `fbx` and `stl`, and that list is short for one reason: **a model
version holds exactly one file.** `gltf` keeps its geometry in a sidecar `.bin` and `obj`
keeps its materials in a sidecar `.mtl`, so either would arrive stripped of whatever the
sidecar held - a model that imports without complaint and is empty or grey. Both are
refused by name, pointing at `glb`, which is glTF in one file. (Blender's self-contained
`GLTF_EMBEDDED` is not a way round it: Blender deprecated that variant and 5.x has removed
it, so a `gltf` target would work on one install and fail on another.)

`stl` succeeds and warns: it carries geometry only, so materials, UVs, rigs, animation and
object names are not in the output. Converting to a format that cannot hold what the source
had is a legitimate thing to want - it just has to say so.

Only one conversion of a version runs at a time. Asking for the same target twice hands
back the same job; asking for a **different** target while one is in flight is refused
rather than deduplicated, because the two produce different files and returning the live
job's id would hand you a format you did not ask for.

**What may be cached is what the geometry hash actually determines, and that is a shorter
list than it looks.** The compute cache is keyed by that hash, which is computed from
_local_ vertex coordinates - so it is blind to UVs **and** blind to the object's transform.
It exists so every copy of the same mesh shares one answer.

Manifoldness and the **local-space** surface area are functions of that geometry alone, so
they go in the cache and `compute_on_demand` can answer them for any asset with that hash.
The cached `surface-area` row carries `"space": "local"` to say so.

Three numbers stay on the job. UV overlap and texel density depend on the UV layout: a model
and the version re-baked from it have **identical geometry, identical hashes and completely
different layouts**, so a cached UV metric would be handed to a mesh it was never measured
on. The reported `surfaceArea` is world-space and so depends on the **transform**: the same
mesh placed at 1x and at 100x hashes identically and has 10,000x the surface. All three come
back on the job, tied to the version and the object actually measured, and
`compute_on_demand` says so rather than answering `pending` forever.

:::note Upgrading from before 0.6

`surface-area` rows written by an earlier build hold the **world**-space number with nothing
recording which instance produced it. There is no conversion back, so those rows are deleted
on migration and `compute_on_demand` treats any unmarked row as `pending`; the next
`analyze_meshes` run fills the cache with a local-space value. Nothing else references the
cache, so the only cost is one recompute.
...
- **`onSurface`** picks _which_ face. A whole-asset top is right for a table and wrong for
  anything with structure: it puts a cushion on the sofa's back, a book above the shelf and a
  tap in mid-air over the sink. Call `get_asset` on what you are stacking **onto**, read its
  `surfaces` list, and pass the `index` of the one you mean. The surface is read once and
  becomes an ordinary anchor offset, so the node still follows what it rests on, undo still
  restores a position, and the editor still drags it like anything else. A wrong index is
  refused with the heights that _are_ available, so the retry is the right one.
