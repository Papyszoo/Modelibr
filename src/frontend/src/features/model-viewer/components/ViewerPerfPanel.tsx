...
/**
 * Live render stats for the model preview. Unlike three-perf / r3f-perf (which
 * read a WebGL context via `getContext().getExtension(...)` and throw on a
 * WebGPURenderer), this reads `renderer.info` + `renderer.backend` directly, so
 * it works on both the WebGPU and the WebGL2 backend the viewer can come up on.
 *
...
export function PerfDisplay({
  ...
  return (
    <div className="viewer-perf-panel" data-testid="viewer-perf-panel">
      <div className="viewer-perf-row viewer-perf-backend-row">
        <span>Backend</span>
        <span
          className={`viewer-perf-badge ${
            stats.backend === 'WebGPU' ? 'is-webgpu' : 'is-webgl'
          }`}
          data-testid="viewer-perf-backend"
        >
          {stats.backend}
        </span>
      </div>
