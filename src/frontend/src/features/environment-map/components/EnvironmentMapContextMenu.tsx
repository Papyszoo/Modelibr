...
>(({ categories = [], tags = [], packId, projectId }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    show: (event, options) => {
      ...
      setMenuMode(
        options.mode ?? (options.environmentMaps.length > 1 ? 'bulk' : 'single')
      )
  ...
  const handleRemoveFromProject = async () => {
    ...
    try {
      await removeEnvironmentMapFromProject(projectId, primaryEnvironmentMap.id)
      await invalidateRelatedQueries()
      toast.current?.show({
        severity: 'success',
        summary: 'Removed',
  ...
  return (
    <>
      ...
      <SelectProjectDialog
        ...
        header={
          isBulkMenu ? `Add ${selectedCountLabel} to Project` : 'Add to Project'
        }
      />
