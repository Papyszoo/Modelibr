...
public class TextureSetCategory : AggregateRoot, IHierarchicalCategory<TextureSetCategory>
{
    ...
    public TextureSetCategory? Parent { get; private set; }
    public ICollection<TextureSetCategory> Children
    {
        get => _children;
        set
        {
    ...
    private static void ValidateName(string name)
    {
        if (string.IsNullOrWhiteSpace(name))
            throw new ArgumentException("Texture set category name cannot be null or empty.", nameof(name));

        if (name.Length > 100)
            throw new ArgumentException("Texture set category name cannot exceed 100 characters.", nameof(name));
    }
