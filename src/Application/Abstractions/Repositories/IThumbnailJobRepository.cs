...
public interface IThumbnailJobRepository
{
    ...
    /// <summary>
    /// Gets a thumbnail job by texture set ID, regardless of status.
    /// </summary>
    Task<ThumbnailJob?> GetByTextureSetIdAsync(int textureSetId, CancellationToken cancellationToken = default);

    /// <summary>
    /// Gets a thumbnail job by environment map variant ID, regardless of status.
    /// </summary>
    Task<ThumbnailJob?> GetByEnvironmentMapVariantIdAsync(int environmentMapVariantId, CancellationToken cancellationToken = default);
