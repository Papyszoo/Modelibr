...
internal class UpdateSpriteCategoryCommandHandler : ICommandHandler<UpdateSpriteCategoryCommand>
{
    ...
    public async Task<Result> Handle(UpdateSpriteCategoryCommand command, CancellationToken cancellationToken)
    {
        var category = await _categoryRepository.GetByIdAsync(command.Id, cancellationToken);
        if (category == null)
        {
            return Result.Failure(
                new Error("CategoryNotFound", $"Sprite category with ID {command.Id} was not found."));
        }
