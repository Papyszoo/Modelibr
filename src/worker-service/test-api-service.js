...
async function testThumbnailApiService() {
  ...
  const testImageData = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAHGbKd2+AAAAABJRU5ErkJggg==', 'base64');
  ...
  try {
    ...
    console.log(`   📋 API endpoint: ${config.apiBaseUrl}/models/${mockModelId}/thumbnail/upload`);
  ...
  try {
    ...
    const result = await apiService.uploadThumbnail(999, '/non/existent/file.png');
