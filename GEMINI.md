...
- **CRITICAL — E2E tests MUST be run after every code change.** Run them with:
    ```bash
    cd tests/e2e
    npm run test
    ```
    This handles container teardown, rebuild, test execution, and cleanup automatically.
    **NEVER** run `npx playwright test` directly or from the repo root — always use `npm run test` inside `tests/e2e/`.
    **NEVER** create temporary output files (`.txt`, `.log`) in the repo when running tests.
